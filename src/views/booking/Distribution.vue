      ...
      <table>
        ...
        <tbody>
          <tr v-for="booking in paginatedBookings" :key="booking.id">
            ...
            <td>{{ formatDate(booking.requested_on) }}</td>
            <td>{{ formatDate(booking.delivery_date) }}</td>
...
const bookingFields = [
  ...
  { label: 'Requested On', name: 'requested_on', type: 'date' },
...
const fetchBookings = async () => {
  try {
    const response = await axios.get(`${API_ENDPOINT}/distributions`);
...
const addBooking = async () => {
  try {
    await axios.post(`${API_ENDPOINT}/distributions`, newBooking.value);
    closeAddModal();
    fetchBookings();
  } catch (error) {
    console.error(error);
...
const updateBooking = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/distributions/${currentBooking.value.id}`, currentBooking.value);
    closeEditModal();
    fetchBookings();
  } catch (error) {
    console.error(error);
...
const deleteBooking = async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/distributions/${bookingToDelete.value.id}`);
    closeDeleteModal();
    fetchBookings();
  } catch (error) {
    console.error(error);
