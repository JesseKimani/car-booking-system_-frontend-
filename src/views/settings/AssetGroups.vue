<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Asset Group</button>
    </nav>
    ...
    <Modal v-if="showAddModal" @close="closeAddModal">
      <template #header>Add Asset Group</template>
      <template #body>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Group</template>
      <template #body>
        <form @submit.prevent="updateRow">
          <div>
            <strong>Name: </strong>
            <input v-model="currentRow.name" placeholder="Group Name" required>
          </div>
          <div>
            <strong>Description: </strong>
    ...
    <Modal v-if="showDeleteModal" @close="closeDeleteModal">
      ...
      <template #body>
        <p>Are you sure you want to delete this asset group?</p>
...
const fetchRows = async () => {
  try {
    const response = await axios.get(`${API_ENDPOINT}/asset_groups`)
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/asset_groups`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/asset_groups/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/asset_groups/${rowToDelete.value.id}`)
