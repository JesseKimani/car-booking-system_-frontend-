<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Expense Type</button>
    </nav>
    ...
    <Modal v-if="showAddModal" @close="closeAddModal">
      <template #header>Add Expense Type</template>
      <template #body>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Expense Type</template>
      <template #body>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Expense Type Details</template>
      <template #body>
        <div v-if="currentRow">
    ...
    <Modal v-if="showDeleteModal" @close="closeDeleteModal">
      <template #header>Delete Expense Type</template>
      <template #body>
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/expense_types`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/expense_types/${currentRow.value.id}`, currentRow.value)
    const index = row.value.findIndex(h => h.id === currentRow.value.id)
    row.value.splice(index, 1, currentRow.value)
    closeEditModal()
  } catch (error) {
    console.error('Error updating expense_type:', error)
  }
}
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/expense_types/${rowToDelete.value.id}`)
