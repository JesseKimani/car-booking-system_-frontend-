<template>
  <div class="settings-table">
    <nav class="navbar">
      <h1>Company Departmets</h1>
      <input v-model="searchQuery" @input="filterTable" placeholder="Search...">
      <button @click="openAddModal">Add Department</button>
    </nav>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Department</template>
      <template #body>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Department Details</template>
      <template #body>
        <div v-if="currentRow">
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/departments`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/departments/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/departments/${rowToDelete.value.id}`)
