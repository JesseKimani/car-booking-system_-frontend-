<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Supplier Category</button>
    </nav>
    ...
    <Modal v-if="showAddModal" @close="closeAddModal">
      <template #header>Add Supplier Category</template>
      <template #body>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Supplier Category</template>
      <template #body>
        <form @submit.prevent="updateRow">
          <div>
            <strong>Name: </strong>
            <input v-model="currentRow.name" placeholder="Supplier Category" required>
          </div>
          <div>
            <strong>Description: </strong>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Category Details</template>
      <template #body>
        <div v-if="currentRow">
...
const fetchRows = async () => {
  try {
    const response = await axios.get(`${API_ENDPOINT}/supplier_categories`)
    row.value = response.data
  } catch (error) {
    console.error('Error fetching supplier cattegory:', error)
  }
}

const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/supplier_categories`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/supplier_categories/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/supplier_categories/${rowToDelete.value.id}`)
    const index = row.value.findIndex(h => h.id === rowToDelete.value.id)
    row.value.splice(index, 1)
    closeDeleteModal()
  } catch (error) {
    console.error('Error deleting supplier categories:', error)
  }
}
