<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Asset Make</button>
    </nav>
    ...
    <Modal v-if="showAddModal" @close="closeAddModal">
      <template #header>Add Asset Make</template>
      <template #body>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Make</template>
      <template #body>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Make Details</template>
      <template #body>
        <div v-if="currentRow">
    ...
    <Modal v-if="showDeleteModal" @close="closeDeleteModal">
      ...
      <template #body>
        <p>Are you sure you want to delete this asset make?</p>
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/assets_makes`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/assets_makes/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/asset_makes/${historyToDelete.value.id}`)
