<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Payment Method</button>
    </nav>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Payment Method</template>
      <template #body>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Expense Payment Method</template>
      <template #body>
        <div v-if="currentRow">
    ...
    <Modal v-if="showDeleteModal" @close="closeDeleteModal">
      <template #header>Delete Payment Method</template>
      <template #body>
...
const fetchRows = async () => {
  try {
    const response = await axios.get(`${API_ENDPOINT}/payment_methods`)
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/payment_methods`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/payment_methods/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/payment methods/${rowToDelete.value.id}`)
