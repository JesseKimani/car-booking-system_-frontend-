<template>
  <div class="settings-table">
    <nav class="navbar">
      ...
      <button @click="openAddModal">Add Fuel Type</button>
    </nav>
    ...
    <Modal v-if="showAddModal" @close="closeAddModal">
      <template #header>Add Fuel Type</template>
      <template #body>
    ...
    <Modal v-if="showEditModal" @close="closeEditModal">
      <template #header>Update Fuel Type</template>
      <template #body>
        <form @submit.prevent="updateRow">
          <div>
            <strong>Name: </strong>
            <input v-model="currentRow.name" placeholder="Fuel Type" required>
          </div>
          <div>
            <strong>Description: </strong>
    ...
    <Modal v-if="showViewModal" @close="closeViewModal">
      <template #header>Expense Fuel Details</template>
      <template #body>
        <div v-if="currentRow">
    ...
    <Modal v-if="showDeleteModal" @close="closeDeleteModal">
      <template #header>Delete Fuel Type</template>
      <template #body>
...
const addRow = async () => {
  try {
    const response = await axios.post(`${API_ENDPOINT}/fuel_types`, newRow.value)
...
const updateRow = async () => {
  try {
    await axios.put(`${API_ENDPOINT}/fuel_types/${currentRow.value.id}`, currentRow.value)
...
const deleteRow= async () => {
  try {
    await axios.delete(`${API_ENDPOINT}/fuel_types/${rowToDelete.value.id}`)
